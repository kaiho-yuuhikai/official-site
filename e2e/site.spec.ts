...
const pages = [
  ...
  { path: '/activities/road-cleaning', name: '道路清掃ボランティア' },
]
...
test('トップページに道路清掃ボランティアセクションが表示される', async ({ page }) => {
  await page.goto('/')
  const section = page.getByRole('heading', { name: '道路清掃ボランティア' })
  await expect(section).toBeVisible()
  const desc = page.getByText('開邦中高校生・PTA・同窓生が合同で')
  await expect(desc).toBeVisible()
  const link = page.getByRole('link', { name: /詳細・活動実績を見る/ })
  await expect(link).toBeVisible()
})

// 道路清掃ボランティアページ: 活動実績
test('道路清掃ボランティアページに第1回活動報告が表示される', async ({ page }) => {
  await page.goto('/activities/road-cleaning')
  await expect(page.getByRole('heading', { name: '道路清掃ボランティア' })).toBeVisible()
  await expect(page.getByRole('heading', { name: '活動実績' })).toBeVisible()
  await expect(page.getByText('第1回')).toBeVisible()
  await expect(page.getByRole('link', { name: /活動報告記事を読む/ })).toBeVisible()
})
